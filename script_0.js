            ...
            try {
                ...
                positions.forEach(p => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td><span style="font-size:11px; padding:2px 6px; border-radius:4px; background:rgba(255,255,255,0.05);">${p.market}
                        <td><span style="color:${p.side === 'BUY' ? 'var(--up-color)' : 'var(--down-color)'}; font-weight:700;">${p.side}
            ...
            if (tbody && recentTrades) {
                tbody.innerHTML = recentTrades.map(t => {
                    ...
                    return `<tr>
                        ...
                        <td><span style="color:${t.action==='BUY'?'var(--up-color)':'var(--down-color)'}; font-weight:700;">${t.action}
                        ...
                        <td>${badge} <span style="color:var(--text-primary); font-size:11px;">${t.lesson_learned}
                    </tr>`;
                }).join('');
            }
        }
        ...
        // YENİ: Sermaye Yönetimi (Capital Management) Slider JS
        const capitalSlider = document.getElementById('capitalSlider');
        const capitalValue = document.getElementById('capitalValue');
        
        if (capitalSlider && capitalValue) {
            capitalSlider.addEventListener('input', function() {
                capitalValue.textContent = '%' + this.value;
            });

            capitalSlider.addEventListener('change', async function() {
                try {
                    const response = await fetch('/api/agent/engine/capital-allocation', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({allocation_pct: parseFloat(this.value)})
                    });
                    const data = await response.json();
                    if (data.status === 'success') {
                        showToast('success', 'Bütçe Güncellendi', data.message);
                    } else {
                        showToast('error', 'Hata', data.message);
                    }
                } catch (err) {
                    showToast('error', 'Bağlantı Hatası', 'Bütçe değiştirilemedi.');
                }
            });
        }
