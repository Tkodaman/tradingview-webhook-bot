...
def run_render_analysis():
    sym = "RENDERUSDT"
    print(f"--- RENDERUSDT 10-SAATLIK ANLIK ANALIZ ---")
    
    # Kripto mum verilerini çek (1 Saatlik mumlar, son 72 saat - 3 gün)
    df = data_fetcher.get_stock_data("RENDER-USD", "1h", 72)
    
    if df is None or df.empty:
        print("HATA: RENDERUSDT verisi alinamadi.")
        return
        
    current_price = df.iloc[-1]['close']
    print(f"Anlik RENDER Fiyati: ${current_price:.4f}")
    ...
    render_tl = current_price * usdtry_price
    print(f"RENDER / TL: {render_tl:.2f} TL")
