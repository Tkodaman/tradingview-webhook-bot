...
# 2. Extract HTML excluding ALL <script> tags inside <body>
idx_body = text.find('<body>')
idx_first_script = text.find('<script>', idx_body)
html_part = text[:idx_first_script]
...
idx_w_body = working.find('<body>')
idx_w_first_script = working.find('<script>', idx_w_body)
