...
# Fix windowNasdaq (currently has crypto notice and gridCrypto)
# Find the crypto notice inside windowNasdaq
...
# Fix windowBist (currently has nasdaq notice and gridNasdaq)
...
# Now swap the notices
# Replace the crypto notice in Nasdaq window with nasdaq notice
text = text.replace(crypto_notice_html, 'REPLACE_ME_NASDAQ')
text = text.replace(bist_notice_html, 'REPLACE_ME_BIST')
text = text.replace(nasdaq_notice_html, 'REPLACE_ME_CRYPTO')
...
with open('templates/dashboard_headers_fixed.html', 'w', encoding='utf-8') as f:
    f.write(text)
